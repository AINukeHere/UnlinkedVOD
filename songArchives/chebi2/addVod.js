#!/usr/bin/env node
/**
 * Add or update a VOD from a Soop URL (chebi2). Run from repo root.
 * Usage: node songArchives/chebi2/addVod.js "https://vod.sooplive.co.kr/player/189435111"
 */
...
const streamerId = 'chebi2';
const url = process.argv[2];

if (!url) {
  console.error('Usage: node songArchives/chebi2/addVod.js "https://vod.sooplive.co.kr/player/{videoId}"');
  process.exit(1);
}
...
async function main() {
  try {
    ...
    if (py.status !== 0) {
      console.error(`Preprocess failed. Run from songArchives: python common/preprocess.py ${streamerId}`);
      process.exit(1);
    }
    console.log('chebi2/songs.js updated.');
  } catch (err) {
