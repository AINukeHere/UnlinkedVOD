...
스트리머별로 노래 기록 보관소 페이지가 있습니다(예: 츄라희 `churahee/`, 체비 `chebi2/`). 각 스트리머는 **자기 data 폴더**의 `config`, `parseConfig`, `source.json`, 레퍼런스만 사용하며, **댓글 파싱 방식**은 `parseConfig.json`(라인 접두사, 시간 정규식, 가수 괄호 추출 등)으로 스트리머마다 다르게 설정할 수 있습니다.
...
프로젝트 **루트**(UnlinkedVOD)에서 실행하세요.

**츄라희**
```bash
npm run churahee:add -- "https://vod.sooplive.co.kr/player/189435111"
# 또는 node churahee/addVod.js "https://vod.sooplive.co.kr/player/189435111"
```

**체비**
```bash
npm run chebi2:add -- "https://vod.sooplive.co.kr/player/{videoId}"
# 또는 node chebi2/addVod.js "https://vod.sooplive.co.kr/player/{videoId}"
```
...
2. 해당 스트리머의 **parseConfig**(`{streamer}/data/parseConfig.json`)로 댓글 라인을 파싱합니다. `linePrefix`, `timeRegex`, `artistFromEndParens` 등을 스트리머마다 다르게 둘 수 있습니다.
3. 옵션으로 해당 스트리머의 **songReference/artistReference**로 제목·가수를 정규화합니다.
4. `{streamer}/data/source.json`에 반영합니다(같은 videoId면 교체, 없으면 추가).
5. `churahee/data/preprocess.py {streamer}`를 실행해 `{streamer}/songs.js`를 다시 만듭니다.
...
로그에서 확인할 수 있는 것: 댓글 총 개수, 설정된 작성자 user_id, 작성자 댓글에서 파싱된 곡 수·목록, 합친 songInfo 개수. 0 songs면 작성자 user_id가 안 맞거나, 댓글이 해당 스트리머의 **parseConfig.linePrefix** 형식이 아닐 수 있음.
